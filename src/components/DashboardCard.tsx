...
export function DashboardCard({ title, onView, onAdd, compact = false, viewCompact = false }: DashboardCardProps) {
    ...
    const viewButtonClass = viewCompact ? "rounded-xl text-[11px] h-7 px-3" : buttonClass;
    ...
    return (
        <div className={containerClass}>
            ...
            <div className={`flex gap-2 ${compact ? "mt-3" : "mt-4"}`}>
                <Button
                    variant="primary"
                    className={`${viewButtonClass} bg-slate-900 text-white hover:bg-slate-800 shadow-md shadow-slate-200`}
                    onClick={onView}
                >
                    View Details
                </Button>
